...
namespace BWEB
{
	...
	TilePosition Map::getBuildPosition(UnitType building, const set<TilePosition>& reservedTiles, TilePosition searchCenter, bool power)
	{
		...
		for (auto &block : blocks)
		{
			...
			for (auto tile : placements)
			{
				if (power)
				{
					if (!Broodwar->hasPower(tile, building))
					{
						continue;
					}
				}
