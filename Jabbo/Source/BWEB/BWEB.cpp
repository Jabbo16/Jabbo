...
namespace BWEB
{
	...
	TilePosition Map::getBuildPosition(UnitType type, TilePosition searchCenter, bool pylonPower)
	{
		double distBest = DBL_MAX;
		...
		for (auto& block : blocks)
		{
			...
			{
				if (pylonPower && !Broodwar->hasPower(tile)) continue;
				const double distToPos = tile.getDistance(searchCenter);
