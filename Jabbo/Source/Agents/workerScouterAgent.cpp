...
namespace Jabbo
{
	...
	void WorkerScouterAgent::getNewHarassTask() const
	{
		...
		for (auto u : InfoManager::instance().getUnits(this->baseToHarass->owner.player)) {
		...
		const auto win = SimulationManager::instance().simulateHarass(this->attackers, this->myUnit, 70);
		if (win)
		{
			this->harassStatus = AttackingWorker;
			return;
	...
	void WorkerScouterAgent::run() const
	{
		...
		case Harassing:
			...
			case Fleeing:
			{
				const auto newFleePosition = MapManager::instance().getKitePosition(attackers, myUnit);
	...
	Unit WorkerScouterAgent::enemyWorkerToHarass() const
	{
		if (baseToHarass)
		{
			...
			for (const auto unit : InfoManager::instance().getUnits(baseToHarass->owner.player))
			{
